...
import space.atnibam.common.service.aop.constant.RateLimiterConstant;
...
public class DistributedRateLimiterNew {
    ...
    public boolean allowRequest() {
        String key = RateLimiterConstant.RATE_LIMIT_KEY + resourceName;
        ...
        // 补充令牌
        long lastRefillTime = redisCache.getCacheObject(RateLimiterConstant.LAST_REFILL_TIME_KEY + resourceName);
        ...
        if (tokenCount > 0) {
            ...
            // 保存令牌桶数量
            redisCache.setCacheObject(key, tokenCount, RateLimiterConstant.TOKEN_BUCKET_EXPIRE_TIME_MINUTES, TimeUnit.MINUTES);
            // 保存最近一次更新token的时间
            redisCache.setCacheObject(RateLimiterConstant.LAST_REFILL_TIME_KEY + resourceName, System.currentTimeMillis(), RateLimiterConstant.TOKEN_BUCKET_EXPIRE_TIME_MINUTES, TimeUnit.MINUTES);
            return true;
        } else {
            return false;
        }
    ...
    private void initializeTokenBucket() {
        // 当资源为空时，则进行新建
        if (redisCache.getCacheObject(RateLimiterConstant.RATE_LIMIT_KEY + resourceName) == null) {
            // 保存最近一次更新token的时间
            redisCache.setCacheObject(RateLimiterConstant.LAST_REFILL_TIME_KEY + resourceName, System.currentTimeMillis(), RateLimiterConstant.TOKEN_BUCKET_EXPIRE_TIME_MINUTES, TimeUnit.MINUTES);
            // 保存令牌桶数量
            redisCache.setCacheObject(RateLimiterConstant.RATE_LIMIT_KEY + resourceName, maxTokens, RateLimiterConstant.TOKEN_BUCKET_EXPIRE_TIME_MINUTES, TimeUnit.MINUTES);
            // 设置过期时间，当长期不用则自动删除
            redisCache.expire(resourceName, RateLimiterConstant.RESOURCE_EXPIRE_TIME_SECONDS);
        }
    }
}
