...
public class RateLimitAspect {
    ...
    public Object aroundRateLimit(ProceedingJoinPoint joinPoint, RateLimit rateLimit) throws Throwable {
        ...
        // 创建并初始化一个分布式限流器
        DistributedRateLimiterNew distributedRateLimiterNew = new DistributedRateLimiterNew(redisCache, resourceName, initialCapacity, refillRate, RateLimiterConstant.REFILL_DURATION);

        // 如果当前请求不能被接受（即超过了设定的频率），则抛出异常
        if (!distributedRateLimiterNew.allowRequest()) {
            throw new RuntimeException(ResultCode.SERVER_LIMIT.getMessage());
        }
