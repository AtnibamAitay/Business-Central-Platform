...
import space.atnibam.transaction.config.WxPayConfig;
import space.atnibam.transaction.constant.WxPayConstant;
...
import space.atnibam.transaction.enums.WxApiType;
import space.atnibam.transaction.enums.WxNotifyType;
import space.atnibam.transaction.factory.PayFactory;
import space.atnibam.transaction.model.dto.AmountDTO;
import space.atnibam.transaction.model.dto.ProductDTO;
import space.atnibam.transaction.model.dto.RequestWxCodeDTO;
...
import space.atnibam.transaction.utils.WechatPay2ValidatorForRequest;
...
import static space.atnibam.transaction.utils.WxPayUtils.sendRequest;
...
public class WxPayNative implements IPayMode {
    ...
    public ResponsePayNotifyDTO paymentNotificationHandler(HttpServletRequest request, OrderStatus successStatus) throws IOException, GeneralSecurityException {
        ...
        if (!wechatPay2ValidatorForRequest.validate(request)) {
            // 若验证失败，记录错误信息并返回错误消息
            log.error("通知验签失败");
            return new ResponsePayNotifyDTO(WxPayConstant.ERROR_CODE, WxPayConstant.ERROR_VALIDATION_FAILED_MSG);
        }
        ...
        // 处理完成，返回成功消息
        return new ResponsePayNotifyDTO(WxPayConstant.SUCCESS_CODE, WxPayConstant.SUCCESS_MSG);
    }
    ...
    private void closeOrder(String orderNo) throws Exception {
        ...
        try {
            ...
            sendRequest(wxPayClient, url, paramsMap, WxPayConstant.CLOSE_ORDER);
        } catch (IOException e) {
