package space.atnibam.transaction.config;
...
public class OrderRabbitConfig {
    ...
    public Exchange orderExchange() {
        return new DirectExchange(RabbitConstant.ORDER_EXCHANGE, true, false);
    }
    ...
    public Exchange orderDlxExchange() {
        return new DirectExchange(RabbitConstant.ORDER_DLX_EXCHANGE, true, false);
    }
    ...
    public Queue orderDelayQueue() {
        ...
        // 设置死信交换机
        args.put("x-dead-letter-exchange", RabbitConstant.ORDER_DLX_EXCHANGE);
        // 设置死信路由Key
        args.put("x-dead-letter-routing-key", RabbitConstant.ORDER_DLQ_ROUTING_KEY);
        // 设置消息过期时间
        args.put("x-message-ttl", RabbitConstant.ORDER_EXPIRE_TIME);
        log.info("延时队列(order.delay.queue)创建");
        return new Queue(RabbitConstant.ORDER_QUEUE, true, false, false, args);
    }
    ...
    public Binding orderDelayQueueBinding() {
        return new Binding(RabbitConstant.ORDER_QUEUE, Binding.DestinationType.QUEUE, RabbitConstant.ORDER_EXCHANGE,
                RabbitConstant.ORDER_QUEUE_ROUTING_KEY, null);
    }
    ...
    public Queue orderCloseQueue() {
        ...
        return new Queue(RabbitConstant.ORDER_DLQ_QUEUE, true, false, false);
    }
    ...
    public Binding orderCloseQueueBinding() {
        return new Binding(RabbitConstant.ORDER_DLQ_QUEUE, Binding.DestinationType.QUEUE, RabbitConstant.ORDER_DLX_EXCHANGE,
                RabbitConstant.ORDER_DLQ_ROUTING_KEY, null);
    }
